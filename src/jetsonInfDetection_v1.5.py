...
parser.add_argument("input_URI1", type=str, default="/media/C2.avi", nargs='?', help="URI of input camera stream")
...
def main():
    ...
    while True:
        ...
        print("Stream1 Detected {:d} objects in image".format(len(detections)))
...
def addToList(obj1, obj2):

    if len(matchListIn) == 160:
        x=matchListIn.pop(0)

    if len(matchListOut) == 160:
        x=matchListOut.pop(0)

    if obj1.direction == "In":
        matchListIn.append(obj1)
    else:
        matchListOut.append(obj1)

    if obj2.direction == "In":
        matchListIn.append(obj2)
    else:
        matchListOut.append(obj2)
...
def checkRoutes(token):
    ...
    for i in matchListOut:
        for j in matchListIn:
            ...
            col1 = i.colour
            col2 = j.colour
            if(class1 == class2 and col1 == col2):
                isMatch = True
                ...
                encoded1 = "{}{}{}{}{}".format(i.source, i.direction, i.colour[0], i.detection.ClassID, i.capTime)
                encoded2 = "{}{}{}{}{}".format(j.source, j.direction, j.colour[0], j.detection.ClassID, j.capTime)
                route = "{}->{}".format(encoded2, encoded1)
		#identifier =
                req.newVehicle(token, route, i.source, i.capTime, j.source, j.capTime)
                return(route)
            else:
                return(isMatch)
